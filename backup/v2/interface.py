...
class Interface():
    ...
    def __init__(self):
        ...
        client_list_column = [
            ...
            [sg.Listbox(values=[], enable_events=True, bind_return_key=True, size=(50, 20), font='ANY 14', key="-CLIENT LIST-")],
    ...
    def event_loop(self):
        ...
        while True:
            ...
            if event == "-CLIENT LIST-":  # Client is chosen from the listbox
                self.window["-CLIENT_FRAME-"].update(activeClient.name)
                self.window['-C_NUM_CHAN-'].update(activeClient.channels)
                self.window['-C_PORT_OFS-'].update(activeClient.port_offset)
            ...
            # try moving this only to areas where it changes
            # it is needed to update connection status check
...
if __name__ == '__main__':
    ...
    # terminate_jacktrip() # clean up existing processes

    # clients = {}
    #
    # default_client = [[sg.Text("create a client")]]
    # self.client_list_column = [
    #     [
    #         sg.Text("Client Name"),
    #         sg.In(size=(25, 1), enable_events=True, key="-CLIENT_NAME_INPUT-"),
    #         sg.Combo([1,2,3,4], default_value=1, key='-NUM_CHANNELS-'),
    ...
    # layout = [
    #     [
    #         sg.Column(self.client_list_column),
    #         sg.VSeperator(),
    #         sg.Column(client_layout.client_viewer_frame),
    #     ]
    # ]
